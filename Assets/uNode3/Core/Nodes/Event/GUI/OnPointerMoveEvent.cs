...
namespace MaxyGames.UNode.Nodes {
	[EventMenu("GUI", "On Pointer Move")]
	[StateEvent]
	public class OnPointerMoveEvent : BaseComponentEvent {
		public ValueInput target { get; set; }
		public ValueOutput value { get; set; }
		private PointerEventData _value;
		...
		public override void OnRuntimeInitialize(GraphInstance instance) {
			...
			if(val != null) {
				if(val is GameObject) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerMove, val as GameObject, (value) => OnTriggered(instance, value));
				}
				else if(val is Component) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerMove, val as Component, (value) => OnTriggered(instance, value));
				}
			}
			else {
				if(instance.target is Component comp) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerMove, comp, (value) => OnTriggered(instance, value));
				}
				else {
