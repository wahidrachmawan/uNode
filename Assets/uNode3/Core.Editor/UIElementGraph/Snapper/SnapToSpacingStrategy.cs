...
namespace MaxyGames.UNode.Editors {
	class SnapToSpacingStrategy : SnapStrategy {
		...
		public override void BeginSnap(UnityEditor.Experimental.GraphView.GraphElement selectedElement) {
			if (IsActive) {
		...
		public override Rect GetSnappedRect(ref Vector2 snappingOffset, Rect sourceRect, UnityEditor.Experimental.GraphView.GraphElement selectedElement, float scale, Vector2 mousePanningDelta = default) {
			if (!IsActive) {
		...
		bool IsIgnoredElement(GraphElement selectedElement, GraphElement element, Rect rectToFit) {
			if (element is Edge || !element.visible || element.IsSelected(m_GraphView) || element.layout.Overlaps(selectedElement.layout) || element is UNodeView nodeView && nodeView.isBlock) {
				return true;
			}
